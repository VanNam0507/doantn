                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach($transactions as $transaction)
                                    ...
                                    <td>
                                        @if($transaction->tr_status==2||$transaction->tr_status==1)
                                            <a class="badge {{$transaction->getStatus($transaction->tr_status)['class']}}" href="">{{$transaction->getStatus($transaction->tr_status)['name']}}</a>
                                        @else
                                            <a class="badge {{$transaction->getStatus($transaction->tr_status)['class']}}" href="{{route('admin.transaction.active',$transaction->id)}}">{{$transaction->getStatus($transaction->tr_status)['name']}}</a>
                                        @endif
                                    </td>
                                    <td>{{$transaction->created_at->format('d-m-Y')}}</td>
                                    <td>
                                        @if($transaction->tr_status==0)
                                        <a class="btn btn-danger" href="{{route('admin.transaction.delete',$transaction->id)}}" ><i class="fas fa-trash"></i> Hủy</a>
                                        @endif
