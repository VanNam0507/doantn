...
<form action="" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="password" value="{{old('password')}}" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Mật khẩu đăng nhập">
    </div>
    @endif
        ...
        <select name="role_id" class="form-control select2_init" >
            ...
            @if(isset($roles))
                @foreach($roles as $role)
                    <option value="{{$role->id}}" {{(isset($admin->role_id) ? $admin->role_id : '') == $role->id ? "selected":"" }}>{{$role->r_display_name}} </option>
                @endforeach
            @endif
        </select>
    </div>
